...
import { ArrowUpRight, Search, FileSpreadsheet, FileText, Download, Calendar, Filter, RefreshCw } from 'lucide-react';
import { StockMovement, MasterBarang } from '../../types';
...
export const HistoryBarangKeluarView: React.FC = () => {
  ...
  useEffect(() => {
    if (realtimeMovements && realtimeMovements.length > 0) {
      const outMovements = realtimeMovements.filter(m => m.jenis === 'Keluar');
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const [resMov, resBrg] = await Promise.all([
        fetch('/api/v1/stock/movements'),
        fetch('/api/v1/stock/barang')
      ]);
      const dataMov = await resMov.json();
      const dataBrg = await resBrg.json();

      if (dataMov.success) {
        const outMovements = (dataMov.data as StockMovement[]).filter(m => m.jenis === 'Keluar');
        setMovements(outMovements);
      }
      if (dataBrg.success) {
        setBarangList(dataBrg.data || []);
      }
    } catch (err) {
      console.error('Error fetching outbound history:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const barangMap = React.useMemo(() => {
    const map: Record<string, MasterBarang> = {};
  ...
  const filtered = movements.filter(m => {
    const matchSearch =
      ...
      (m.penerimaTujuan && m.penerimaTujuan.toLowerCase().includes(search.toLowerCase())) ||
  ...
  const getItemSatuan = (m: StockMovement) => m.satuan || barangMap[m.barangId]?.satuan || 'Pcs';
  const getItemSisaStock = (m: StockMovement) => m.sisaStock !== undefined ? m.sisaStock : (barangMap[m.barangId]?.stokSekarang ?? 0);
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(14);
    doc.text('LAPORAN HISTORY BARANG KELUAR', 14, 15);
    doc.setFontSize(9);
    doc.text(`Dicetak pada: ${new Date().toLocaleString('id-ID')} | Total Item: ${filtered.length}`, 14, 22);

    const tableColumn = ['No', 'Tanggal & Jam', 'Nama Barang', 'Satuan', 'Jumlah Keluar', 'Harga Satuan (Rp)', 'Total Nilai (Rp)', 'Sisa Stock', 'Petugas'];
    const tableRows = filtered.map((m, idx) => [
      ...
      getItemSatuan(m),
      m.jumlah,
      (m.hargaSatuan || 0).toLocaleString('id-ID'),
    ...
    doc.save(`History_Barang_Keluar_${new Date().toISOString().split('T')[0]}.pdf`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300 text-[10px] font-extrabold uppercase rounded-full">
              Audit Outbound Log
            </span>
            <span className="text-xs text-slate-500">Pengeluaran Bahan Baku & Distribusi Dapur / Operasional</span>
          </div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            <ArrowUpRight className="w-6 h-6 text-rose-600" /> Riwayat Transaksi Barang Keluar
          </h2>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs">
          <span className="text-xs font-medium text-slate-500 block">Total Transaksi Keluar</span>
          <span className="text-2xl font-black text-slate-900 dark:text-slate-100">{filtered.length} Transaksi</span>
        </div>
        <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs">
          <span className="text-xs font-medium text-slate-500 block">Total Volume Barang Keluar</span>
          <span className="text-2xl font-black text-rose-600 dark:text-rose-400">-{totalVolume.toLocaleString('id-ID')} Pcs/Kg</span>
        </div>
        <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs">
          <span className="text-xs font-medium text-slate-500 block">Total Nilai Outbound (Rp)</span>
      ...
      <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs flex flex-col md:flex-row items-center gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="Cari nota, barang, tujuan/penerima, petugas..."
            value={search}
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 font-semibold">
              ...
              <th className="p-3 text-right">Jumlah Keluar</th>
              <th className="p-3 text-right">Harga Satuan</th>
              <th className="p-3 text-right">Total Nilai</th>
              <th className="p-3 text-right">Sisa Stock</th>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={9} className="p-8 text-center text-slate-400 italic">
                  Tidak ada riwayat barang keluar ditemukan.
                </td>
              </tr>
            ) : (
              filtered.map((m, idx) => (
                <tr key={m.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition">
                  ...
                  <td className="p-3 font-mono text-[11px] text-slate-600 dark:text-slate-400">{m.tanggal}</td>
                  <td className="p-3">
                    <div className="font-bold text-slate-900 dark:text-slate-100">{m.namaBarang}</div>
                    <div className="text-[10px] text-slate-400 font-mono">{m.kodeBarang}</div>
                  </td>
                  <td className="p-3 font-medium text-slate-600 dark:text-slate-400">{getItemSatuan(m)}</td>
                  <td className="p-3 text-right font-black text-rose-600 dark:text-rose-400">
                    -{m.jumlah}
                  </td>
                  <td className="p-3 text-right font-medium">Rp {(m.hargaSatuan || 0).toLocaleString('id-ID')}</td>
                  ...
                  <td className="p-3 text-right font-bold text-blue-600 dark:text-blue-400">
                    {getItemSisaStock(m)} {getItemSatuan(m)}
                  </td>
                  <td className="p-3 text-slate-600 dark:text-slate-400 font-medium">{m.petugas}</td>
                </tr>
              ))
            )}
          </tbody>
